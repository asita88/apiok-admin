...
type Upstreams struct {
	...
	CheckEnabled   int    `gorm:"column:check_enabled"`   // Health check enabled  0:false  1:true
	CheckTcp       int    `gorm:"column:check_tcp"`       // Health check type  0:http  1:tcp
	CheckMethod    string `gorm:"column:check_method"`    // HTTP method for health check
	CheckHost      string `gorm:"column:check_host"`      // HTTP Host header for health check
	CheckUri       string `gorm:"column:check_uri"`       // HTTP URI for health check
	CheckInterval  int    `gorm:"column:check_interval"`  // Health check interval in seconds
	CheckTimeout   int    `gorm:"column:check_timeout"`   // Health check timeout in seconds
	ModelTime
}
